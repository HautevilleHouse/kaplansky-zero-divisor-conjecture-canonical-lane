...
In-paper anchor: `paper/KAPLANSKY_ZERO_DIVISOR_CONJECTURE_PREPRINT.md` (`KZD_G3`).

## Goal
Make the compactness/no-Zeno gate explicit for `proving zero-divisor exclusion for torsion-free group rings through an admissible support-persistence closure architecture`: near-failure behavior must be captured, not allowed to drift or accumulate invisibly.
...
`KZD_G3` closes when `kappa_compact` provides compactness/no-collapse control: normalized near-failure families are precompact and support windows do not collapse.
This is the first-failure capture contribution to `M_KZD`.
...
If near-failure extraction and no-Zeno spacing hold, then `KZD_G3` prevents hidden drift before rigidity is applied.
