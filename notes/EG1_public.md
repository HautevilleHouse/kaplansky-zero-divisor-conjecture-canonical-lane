...
In-paper anchor: `paper/KAPLANSKY_ZERO_DIVISOR_CONJECTURE_PREPRINT.md` (`KZD_G1`).

## Goal
Make the projected response floor explicit as the protected-core gate for `proving zero-divisor exclusion for torsion-free group rings through an admissible support-persistence closure architecture`.
...
- projected core: the response sector controlled by `kappa_support`.
...
`KZD_G1` closes when `kappa_support` satisfies the response-floor requirement: projected support response has a strict positive floor.
This is the protected-core contribution to the strict margin `M_KZD`.
...
Payload: verify that all quantities used by `kappa_support` are defined on the projected sector named in the main preprint.
...
Payload: check the artifact key `kappa_support` and the corresponding extraction input/provenance record.

### Theorem EG1.3 (core gate closure)
If Lemmas EG1.1-EG1.2 hold and the runtime artifact accepts `kappa_support`, then `KZD_G1` supplies the projected/protected-core input to `M_KZD`.

## Current Instantiation

- gate: `KZD_G1`
- artifact key: `kappa_support`
