...
In-paper anchor: `paper/KAPLANSKY_ZERO_DIVISOR_CONJECTURE_PREPRINT.md` (bridge and margin interface).

## Goal
Identify the canonical endpoint representative with the ordinary problem-side target for `proving zero-divisor exclusion for torsion-free group rings through an admissible support-persistence closure architecture`.
...
2. the endpoint-transfer lock `domain_transfer` identifies the surviving representative;
3. the coherence remainder `eps_coh` is not smuggled into the endpoint;
4. the strict closure margin `M_KZD` remains positive.
...
Payload: verify `domain_transfer` against the constants registry and runtime certificate.
...
If lock persistence, determining-class uniqueness, and strict coherence hold, then the endpoint representative is identified and the final margin `M_KZD` is meaningful.

## Current Instantiation

- endpoint-transfer artifact key: `domain_transfer`
