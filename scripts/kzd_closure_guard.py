#!/usr/bin/env python3
"""Canonical-lane closure guard for the kaplansky zero-divisor conjecture workspace."""
...
def compute_report(data: dict[str, Any], sigma_star: float | None, strict_coh_zero: bool) -> dict[str, Any]:
    ...
    native_names = ['KZD_G1', 'KZD_G2', 'KZD_G3', 'KZD_G4', 'KZD_G5']
    gates = {name: ('PASS' if passed else 'FAIL') for name, passed in zip(native_names, gate_pass)}
    gates['KZD_G6'] = 'PASS' if coh_pass else 'FAIL'
    gates['KZD_GM'] = 'PASS' if gm_pass else 'FAIL'
    ...
    blockers['KZD_G6'] = [] if coh_pass else ['eps_coh missing/invalid or strict-zero target failed']
    blockers['KZD_GM'] = [] if gm_pass else ['strict margin <= 0 or upstream gates failed']
    ...
    report = {
        'meta': {
            ...
            'framework': 'kaplansky_zero_divisor_conjecture',
        },
        'schema': {
